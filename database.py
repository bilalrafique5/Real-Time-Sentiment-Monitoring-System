...
def get_cached_tweets(query: str, limit: int = 100):
    """
    Return cached tweets for this query if they were inserted within CACHE_HOURS
    """
    conn = get_conn()
    ...
    q = """
    SELECT tweet_id, query, date, username, content, clean_text,
           vader_label, vader_score, distil_label, distil_score, inserted_at
    FROM tweets
    WHERE query = ?
      AND inserted_at >= ?
